...
public class Legal_IBM_BPQuote_TC07 extends Driver_Setup{
	...
	public void Legal_IBM_BPQuote_TC07_main(String IBMURL, String ENV) throws Throwable {
		try {
		...
		//	EpricerApplicationSearchQuoteBusinessLogic.selectactioninIBMsearch("Open");
			...
			Thread.sleep(10000);
			boolean checkresult = EpricerApplicationIBMQuoteEditorBusinessLogic.checkchangestatusdisplayed("Incomplete",true);
				
			System.out.println("check result for user is " + checkresult);

			if (checkresult) {

				Extent_Reporting.Log_Pass("Legal_IBM_BPQuote_TC07 Passed.", "Legal_IBM_BPQuote_TC07 Passed.", test);
			} else {
			...
			Extent_Reporting.Log_Fail("Legal_IBM_BPQuote_TC07 Failed.",
					"Legal_IBM_BPQuote_TC07 Failed", driver, test);
			driver.quit();
			e.printStackTrace();
			throw new Exception("Failed");
		}
