...
public class Legal_IBM_Transaction_TC02 extends Driver_Setup {
	...
	public void Legal_IBM_Transaction_TC02_main(String IBMURL, String ENV) throws Throwable {
		try {
			...
			EpricerApplicationIBMQuoteEditorBusinessLogic.ClickMyQuoteLink();
			// EpricerApplicationIBMQuoteEditorBusinessLogic.FilterOnGoingonMyQuote();

			Thread.sleep(5000);
			EpricerApplicationSearchQuoteBusinessLogic.checkonthefirstresult();
			Thread.sleep(5000);
			boolean checkresult = EpricerApplicationSearchQuoteBusinessLogic.Checkdropdownlistinibmsearch("Delete",true);

			if (checkresult) {
				Extent_Reporting.Log_Pass("Legal_IBM_Transaction_TC02 Passed.", "Legal_IBM_Transaction_TC02 Passed.",test);
			} else {
				throw new Exception("Failed");
			}
		} catch (Exception e) {
			Extent_Reporting.Log_Fail("Legal_IBM_Transaction_TC02 Failed.", "Legal_IBM_Transaction_TC02 Failed", driver,test);
			driver.quit();
			e.printStackTrace();
			throw new Exception("Failed");
		}
