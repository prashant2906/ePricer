...
public class Legal_IBM_Transaction_TC61 extends Driver_Setup{
	...
	public void Legal_IBM_Transaction_TC61_main(String IBMURL, String ENV) throws Throwable {
		try {
		...
		*/		
			...
			EpricerApplicationSearchQuoteBusinessLogic.searchCriteriaQuotestatussubcriteriaBPreasoncode(false,"Special bid verification");
		...
		//	EpricerApplicationSearchQuoteBusinessLogic.selectactioninIBMsearch("Open");
			...
			Thread.sleep(5000);
			boolean checkresult =  EpricerApplicationIBMQuoteEditorBusinessLogic.checkdropdwonlistforstatuschange("Incomplete",false);
			
						
			if(checkresult)
			{
				
				Extent_Reporting.Log_Pass("Legal_IBM_Transaction_TC61 Passed.",
    					"Legal_IBM_Transaction_TC61 Passed.", test);
			}
			else
			{
			...
			Extent_Reporting.Log_Fail("Legal_IBM_Transaction_TC61 Failed.",
					"Legal_IBM_Transaction_TC61 Failed", driver, test);
			driver.quit();
			e.printStackTrace();
			throw new Exception("Failed");
		}
