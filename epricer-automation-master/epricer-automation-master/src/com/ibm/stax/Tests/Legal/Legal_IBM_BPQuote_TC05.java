...
public class Legal_IBM_BPQuote_TC05 extends Driver_Setup{
	...
	public void Legal_IBM_BPQuote_TC05_main(String IBMURL, String ENV) throws Throwable {
		try {
        ...
        PCSloginBusinessLogic.authenticatePopUpHandle(IBMURL,ENV);
			...
			boolean checkresult = EpricerApplicationSearchQuoteBusinessLogic.Checkquoteypedisplayed("bp",true);
		
			System.out.println("check result for user is " + checkresult);

			if (checkresult) {

				Extent_Reporting.Log_Pass("Legal_IBM_BPQuote_TC05 Passed.", "Legal_IBM_BPQuote_TC05 Passed.", test);
			} else {
			...
			Extent_Reporting.Log_Fail("Legal_IBM_BPQuote_TC05 Failed.",
					"Legal_IBM_BPQuote_TC05 Failed", driver, test);
			driver.quit();
			e.printStackTrace();
			throw new Exception("Failed");
		}
