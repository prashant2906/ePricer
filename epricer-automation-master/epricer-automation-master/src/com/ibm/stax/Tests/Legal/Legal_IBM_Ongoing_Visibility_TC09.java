...
public class Legal_IBM_Ongoing_Visibility_TC09 extends Driver_Setup{
	...
	public void Legal_IBM_Ongoing_Visibility_TC09_main(String IBMURL, String ENV) throws Throwable {
		try {
			...
			PCSloginBusinessLogic.authenticatePopUpHandle(IBMURL,ENV);
			PCSloginBusinessLogic.ePricerLoginscreen("SelectIBMRole", ENV); // just use this column as new ibm group
			...
			if(checkIfQuoteExists1)
			{
				Extent_Reporting.Log_Pass("Legal_IBM_Ongoing_Visibility_TC09 Passed.",
    					"Legal_IBM_Ongoing_Visibility_TC09 Passed.", test);
			}
			else
			{
			...
			Extent_Reporting.Log_Fail("Legal_IBM_Ongoing_Visibility_TC09 Failed.",
					"Legal_IBM_Ongoing_Visibility_TC09 Failed", driver, test);
			driver.quit();
			e.printStackTrace();
			throw new Exception("Failed");
		}
