...
public class Legal_IBM_Transaction_TC27 extends Driver_Setup {
	...
	public void Legal_IBM_Transaction_TC27_main(String IBMURL, String ENV) throws Throwable {
		try {
			...
			boolean checkresult = EpricerApplicationIBMQuoteEditorBusinessLogic
					.checkpricelevelonDisplayCriteriaPage("Best price", true);

			System.out.println("check result for user is " + checkresult);

			if (checkresult) {
				Extent_Reporting.Log_Pass("Legal_IBM_Transaction_TC27 Passed.", "Legal_IBM_Transaction_TC27 Passed.",
						test);
			} else {
				throw new Exception("Failed");
			}
		} catch (Exception e) {
			Extent_Reporting.Log_Fail("Legal_IBM_Transaction_TC27 Failed.", "Legal_IBM_Transaction_TC27 Failed", driver,
					test);
			driver.quit();
			e.printStackTrace();
			throw new Exception("Failed");
