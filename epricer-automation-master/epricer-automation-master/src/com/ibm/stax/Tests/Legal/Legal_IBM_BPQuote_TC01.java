...
public class Legal_IBM_BPQuote_TC01 extends Driver_Setup{
	...
	public void Legal_IBM_BPQuote_TC01_main(String IBMURL, String ENV) throws Throwable {
		try {
		...
		PCSloginBusinessLogic.authenticatePopUpHandle(IBMURL,ENV);
			...
			boolean checkresult = EpricerApplicationIBMQuoteEditorBusinessLogic.checklinkondashboard("Duplicate screening",true);
			if(checkresult)
			{
				
				Extent_Reporting.Log_Pass("Legal_IBM_BPQuote_TC01 Passed.",	"Legal_IBM_BPQuote_TC01 Passed.", test);
			}
			else
			{
			...
			Extent_Reporting.Log_Fail("Legal_IBM_BPQuote_TC01 Failed.","Legal_IBM_BPQuote_TC01 Failed", driver, test);
			driver.quit();
			e.printStackTrace();
			throw new Exception("Failed");
		}
