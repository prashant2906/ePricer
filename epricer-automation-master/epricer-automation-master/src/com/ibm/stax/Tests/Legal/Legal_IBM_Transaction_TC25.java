...
public class Legal_IBM_Transaction_TC25 extends Driver_Setup {
	...
	public void Legal_IBM_Transaction_TC25_main(String IBMURL, String ENV) throws Throwable {
		try {
		...
		PCSloginBusinessLogic.authenticatePopUpHandle(IBMURL,ENV);
			...
			boolean checkresult = EpricerApplicationIBMQuoteEditorBusinessLogic.checklinkdisplayed("Prices", true);

			System.out.println("check result for user is " + checkresult);

			if (checkresult) {
				Extent_Reporting.Log_Pass("Legal_IBM_Transaction_TC25 Passed.", "Legal_IBM_Transaction_TC25 Passed.",
						test);
			} else {
				throw new Exception("Failed");
			}
		} catch (Exception e) {
			Extent_Reporting.Log_Fail("Legal_IBM_Transaction_TC25 Failed.", "Legal_IBM_Transaction_TC25 Failed", driver,
					test);
			driver.quit();
			e.printStackTrace();
			throw new Exception("Failed");
