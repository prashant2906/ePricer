...
public class Legal_IBM_Transaction_TC06 extends Driver_Setup {
	...
	public void Legal_IBM_Transaction_TC06_main(String IBMURL, String ENV) throws Throwable {
		try {
			...
			// the user can not see the level 1 price now
			boolean checkresult = EpricerApplicationIBMQuoteEditorBusinessLogic.checkpricetypeshown("Level 2", false);
			System.out.println("check result for user is " + checkresult);

			if (checkresult) {
				Extent_Reporting.Log_Pass("Legal_IBM_Transaction_TC06 Passed.", "Legal_IBM_Transaction_TC06 Passed.",
						test);
			} else {
				throw new Exception("Failed");
			}
		} catch (Exception e) {
			Extent_Reporting.Log_Fail("Legal_IBM_Transaction_TC06 Failed.", "Legal_IBM_Transaction_TC06 Failed", driver,
					test);
			driver.quit();
			e.printStackTrace();
			throw new Exception("Failed");
