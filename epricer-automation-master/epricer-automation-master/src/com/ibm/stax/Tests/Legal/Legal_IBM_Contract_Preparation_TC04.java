...
public class Legal_IBM_Contract_Preparation_TC04 extends Driver_Setup {
	...
	public void Legal_IBM_Contract_Preparation_TC04_main(String IBMURL, String ENV) throws Throwable {
		try {
			...
			EpricerApplicationSearchQuoteBusinessLogic.selectactioninIBMsearch("Replace");
			Thread.sleep(15000);
			EpricerApplicationSearchQuoteBusinessLogic.duplicateattachment(false);

			Thread.sleep(20000);
			EpricerApplicationIBMQuoteEditorBusinessLogic.switchtotabinIBM("Overview");
			...
			EpricerApplicationIBMQuoteEditorBusinessLogic.actiononAdmintab("prepare");

			// EpricerApplicationIBMQuoteEditorBusinessLogic.closecurrentIBMquote();

			Thread.sleep(5000);
			Boolean checkQuotestatus = createAQuoteBusinessLogic.checkQuotestatus("Contract preparation");

			if (checkQuotestatus) {
				Extent_Reporting.Log_Pass("Legal_IBM_Contract_Preparation_TC04 Passed.",
						"Legal_IBM_Contract_Preparation_TC04 Passed.", test);
			} else {
				throw new Exception("Failed");
			}

		} catch (Exception e) {
			Extent_Reporting.Log_Fail("Legal_IBM_Contract_Preparation_TC04 Failed.",
					"Legal_IBM_Contract_Preparation_TC04 Failed", driver, test);
			driver.quit();
			e.printStackTrace();
			throw new Exception("Failed");
		}
