...
public class Legal_IBM_Transaction_TC33 extends Driver_Setup{
	...
	public void Legal_IBM_Transaction_TC33_main(String IBMURL, String ENV) throws Throwable {
		try {
		...
		*/		
		...
		//	EpricerApplicationSearchQuoteBusinessLogic.selectactioninIBMsearch("Open");
			...
			EpricerApplicationIBMQuoteEditorBusinessLogic.switchtotabinIBM("ReportsLetters");
			Thread.sleep(5000);
				
			boolean checkresult =  EpricerApplicationIBMQuoteEditorBusinessLogic.checkbuttondisplayed("approvalhistory",true);
				
			if(checkresult)
			{
				
				Extent_Reporting.Log_Pass("Legal_IBM_Transaction_TC33 Passed.",
    					"Legal_IBM_Transaction_TC33 Passed.", test);
			}
			else
			{
			...
			Extent_Reporting.Log_Fail("Legal_IBM_Transaction_TC33 Failed.",
					"Legal_IBM_Transaction_TC33 Failed", driver, test);
			driver.quit();
			e.printStackTrace();
			throw new Exception("Failed");
		}
