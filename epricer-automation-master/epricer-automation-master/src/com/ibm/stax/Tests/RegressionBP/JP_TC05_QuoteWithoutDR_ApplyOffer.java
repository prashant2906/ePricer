...
public class JP_TC05_QuoteWithoutDR_ApplyOffer extends Driver_Setup{
		...
		try {
          ...
          //  createAQuoteBusinessLogic.acceptValueSellerOfferButtonClicked();
           // createAQuoteBusinessLogic.valueSellerApprovedStatusCheck();
			...
			Extent_Reporting.Log_Pass("Quote_With_DR_ApplyOffer Passed.",
					"Quote_With_DR_ApplyOffer Passed.",test);
		} catch (Exception e) {
			Extent_Reporting.Log_Fail("Quote_With_DR_ApplyOffer Failed.",
					"Quote_With_DR Failed_ApplyOffer.", driver,test);
			driver.quit();
			e.printStackTrace();
